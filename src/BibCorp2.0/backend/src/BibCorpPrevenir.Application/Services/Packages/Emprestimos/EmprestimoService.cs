...
namespace BibCorpPrevenir.Application.Services.Packages.Emprestimos
{
  public class EmprestimoServices : IEmprestimoServices
  {
    ...
    public async Task<EmprestimoDto> GerenciarEmprestimos(int emprestimoId)
    {
      try
      {
        ...
        if ( emprestimoAlterado.Acao == TipoAcaoEmprestimo.Devolver || emprestimoAlterado.Acao == TipoAcaoEmprestimo.Recusar)
        {
